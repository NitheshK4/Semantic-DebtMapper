...
class MitigationService:
    """Service to execute the actual database updates required to resolve
    semantic debt findings.
    """

    @staticmethod
    def mitigate_action(
        db: Session, project_id: UUID, action_card: ActionCard
    ) -> bool:
        """Applies database changes corresponding to the action type to
        resolve the underlying drift.
        """
        run_id = action_card.run_id
        action_type = action_card.action_type

        logger.info(
            f"Mitigating action {action_card.id} of type {action_type} "
        ...
        if not quoted_vars:
            logger.warning(
                f"Could not extract target variables from title: {action_card.title}"
            )
            return False
        ...
        if action_type == "RELABEL_SUBSET":
            ...
            # Reconcile label schemas: update definition of class_id in all
            # schemas to match the latest one
            latest_schema = (
                db.query(LabelSchema)
                .filter(LabelSchema.project_id == project_id)
                .order_by(LabelSchema.effective_from.desc())
                .first()
            )
            if latest_schema:
                latest_class_def = next(
                    (
                        c
                        for c in latest_schema.payload.get("classes", [])
                ...
                if latest_class_def:
                    earlier_schemas = (
                        ...
                        .filter(
                            ...
                            LabelSchema.id != latest_schema.id,
                        )
                        .all()
                    )
                    ...
                    db.commit()
                    logger.info(
                        f"Reconciled definitions for class '{class_id}' "
                        f"across all schemas."
                    )
                    return True
            return False
        ...
        # 2. ESF: Rebuild embedding index / Update index version metadata
            ...
            if models:
                ...
                db.commit()
                logger.info(
                    f"Updated index version metadata for {endpoint_id} "
                    f"to match {active_model.model_version}."
                )
                return True
            return False
                    ...
                    for rule in rules:
                        ...
                        if "score >= 0.82" in rule.expression:
                            rule.expression = rule.expression.replace(
                                "score >= 0.82", "score >= 0.76"
                            )
                        elif "score >= 0.85" in rule.expression:
                            rule.expression = rule.expression.replace(
                                "score >= 0.85", "score >= 0.76"
                            )
                        elif "score >= 0.95" in rule.expression:
                            rule.expression = rule.expression.replace(
                                "score >= 0.95", "score >= 0.89"
                            )
                        db.add(rule)
                    db.commit()
                    logger.info(
                        f"Recalibrated rule '{rule_id}' to model '{current_model}'."
                    )
                    return True
            return False
            ...
            if finding:
                ...
                for ovr in overrides:
                    ...
                    if inf:
                        ...
                        s1 = (
                            json.loads(inf.segment)
                            if isinstance(inf.segment, str)
                            else inf.segment
                        )
                        s2 = (
                            json.loads(segment)
                ...
                db.commit()
                logger.info(
                    f"Deleted {count} manual overrides in segment {segment} "
                    f"to reconcile class '{class_id}'."
                )
                return True
            return False
        ...
        # 5. GFM: Retire feature from rule expression
            ...
            if rule_id:
                ...
                db.commit()
                logger.info(
                    f"Retired feature '{feature}' from business rule '{rule_id}'."
                )
                return True
            return False
